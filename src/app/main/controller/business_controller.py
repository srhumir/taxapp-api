...
api = BusinessDto.api
_new_business = BusinessDto.new_business
_existing_business = BusinessDto.existing_business


@api.route('/')
class BusinessList(Resource):
    @api.doc('list_of_registered_businesss')
    @api.marshal_list_with(_existing_business, envelope='data')
    def get(self):
    ...
    @api.expect(_new_business, validate=True)
    def post(self):
    ...
    @api.expect(_existing_business, validate=True)
    def put(self):
        """Edit an existing business"""
        data = request.json
        return edit_business(data=data)
...
class Customer(Resource):
    ...
    @api.marshal_with(_existing_business)
...
class Customer(Resource):
    ...
    @api.marshal_list_with(_existing_business)
